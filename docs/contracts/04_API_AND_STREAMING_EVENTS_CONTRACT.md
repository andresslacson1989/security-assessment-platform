...
- `POST /api/scans/{scan_id}/cancel`: Requests cancellation of an active scan (`scan:cancel`) through the shared execution cancellation coordinator. The endpoint returns `cancelled: true` and broadcasts `event: cancelled` only after task shutdown, exact child-process termination, and durable child-run settlement are confirmed. If the supervisor returns `NOT_FOUND` or `FAILED`, or the owning task/recovery state cannot be joined, the response preserves the current scan status, returns `cancelled: false` and `cancellation_pending: true`, records an operator-visible pending audit result, and does not claim terminal cancellation.
...
- `POST /api/system/executions/{request_id}/revoke`: Revokes approval or stops the associated execution through the shared cancellation coordinator. The response identifies the exact execution, typed `cancellation_status`, `durable_terminal`, and `recovery_required` state; `NOT_FOUND` and `FAILED` remain recoverable and are not clean terminal results.
