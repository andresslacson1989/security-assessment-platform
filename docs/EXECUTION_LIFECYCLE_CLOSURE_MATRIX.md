...
| Single cancellation coordinator | One coordinator owns cancellation request, task shutdown, process termination, authority revocation, and terminal settlement. On the durable-identity production path, a timed-out exact supervisor cancellation retains its real concurrent-future ownership; it is keyed by execution, tenant, exact process-proof fingerprint, recovery generation, lease/attempt fingerprint, and cannot be duplicated by a later cadence. A shared execution-plus-organization pending-operation gate reconciles completed futures before an uncertain recovery lease claim or governed supervisor submission; an unfinished future leaves the durable projection retryable and blocks a new lease, attempt, or invocation. A late callback may record reconciliation-needed evidence but cannot terminalize or rewrite the run. Legacy compatibility doubles without the durable identity API are also future-owned and cannot terminalize from a late result, but they do not provide exact-process assurance. | Disposable SQLite vectors for timeout through bounded shutdown, governed and uncertain duplicate-cadence fencing, late `KILLED` non-terminal behavior, late failure consumption, and retry after completion, plus existing cancellation/idempotence vectors. PostgreSQL, deployed, CI, and independent acceptance remain required. | Local Section B evidence recorded in `docs/evidence/section_b_durable_recovery_local_2026-09-14.md`; OPEN |
| Durable recovery | Recovery attempts, status, bounded retry/backoff, next attempt, and escalation are persisted by execution ID and organization. A database mutation or supervisor cancellation that outlives an async waiter retains an owned concurrent-future handle; cancellation, timeout, or bounded shutdown never cancels or silently replaces actual work. Same-identity joins require a complete request fingerprint; conflicts reject. Before an uncertain path claims a new durable lease, and before a governed path submits a cancellation, the shared pending-operation gate reconciles completed supervisor futures and blocks unfinished exact identity work without mutating its retryable durable projection. A fresh lifecycle reconciles only from the durable tenant-bound recovery projection. Durable-identity production cancellation binds to exact process proof; the legacy compatibility path remains conservative and non-attested. | Disposable SQLite vectors for database-write and supervisor timeout/cancellation/shutdown, late success/failure consumption, governed and uncertain same-identity conflict fencing, fresh-lifecycle visibility, closed event-loop database completion, and authenticated tenant-scoped recovery health. PostgreSQL row-lock/concurrency, deployed non-empty recovery, CI, and independent acceptance evidence remain required. | Local Section B evidence recorded in `docs/evidence/section_b_durable_recovery_local_2026-09-14.md`; implementation, CI, deployed recovery proof, and independent acceptance remain OPEN |
...
The current corrective candidate adds one shared in-memory pending-operation
gate to the observer's actual recovery path. The gate is keyed by the exact
`execution_id` and `organization_id`, consumes completed owned supervisor
futures before evaluation, and blocks an unfinished future before either an
uncertain recovery lease/attempt claim or a governed supervisor cancellation
submission. It leaves the durable retryable projection unchanged and records
bounded reconciliation-needed evidence. The corresponding durable uncertain
path vector is
`tests/test_observation_service.py::test_uncertain_supervisor_timeout_does_not_claim_second_recovery_lease`.
This is a local implementation checkpoint only; it does not change the
acceptance status or provide PostgreSQL, deployment, CI, mirror, or independent
review evidence.
...
| Contract 04 §1.5 and lifecycle requirements — durable recovery and operator visibility | The quarantine API vectors use real disposable admin identities and the authenticated route to reject unauthenticated access, enforce tenant scoping, expose observer-created deferred recovery state, and omit process-identity fields. Observation-service vectors cover bounded retries, missing identity, uncertain state, durable settlement, and late-write reconciliation. | Local evidence added; CT108 production endpoint remains health-only with no live recovery rows; CI and independent acceptance remain required |
| Contract 08 §6.1.2 — worker restart, recovery, and graceful shutdown | The observer vectors prove a started durable write remains owned through timeout, caller cancellation, bounded shutdown, closed event-loop callback failure, and a fresh lifecycle; they also reject conflicting same-identity payloads and fence restart while a cancellation-resistant recovery worker remains unresolved. The worker launch-inventory vector covers awaited signal shutdown. CT108 restart evidence is exact-source health evidence only and did not exercise a non-empty recovery queue. | Local evidence added; OPEN pending CI, independent acceptance, and non-empty deployed recovery proof |
