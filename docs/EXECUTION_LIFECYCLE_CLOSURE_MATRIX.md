...
The accepted baseline has exact GitHub Actions evidence in run
`34659175863`, including the Windows Job Object assurance job
`103457915354`. The current follow-up still requires a fresh GitHub run for
its exact grouped delivery commit. The matrix remains open because worker-
generation deployment binding, current-candidate live Redis/PostgreSQL
evidence, broader independent OS-level process evidence, managed-tool runtime
evidence, and independent auditor acceptance are not closed by local tests.
