...
| Contract 04 §1.5/lifecycle — authenticated, tenant-scoped recovery visibility | `tests/security/test_execution_quarantine_api.py::test_recovery_health_is_authenticated_tenant_scoped_and_identity_safe` proves real disposable admin JWT tenant isolation and process-identity omission. `tests/security/test_execution_quarantine_api.py::test_recovery_health_exposes_observer_created_state_from_production_revocation` proves an observer-created deferred record crosses the authenticated production route while unauthenticated access remains rejected and a second tenant cannot view it. `tests/security/test_execution_quarantine_api.py::test_recovery_health_exposes_supervisor_timeout_from_production_observer` proves a production-observer supervisor timeout is exposed as tenant-scoped `DEFERRED/termination_timeout` evidence without exposing process identity. | Local evidence added; CT108 remains health-only because it has no live recovery rows; CI and independent acceptance remain open |
| Contract 08 §6.1.2 — restart-safe recovery and graceful worker shutdown | `backend/app/core/observation_service.py::_durable_recovery_write`, `_get_or_submit_durable_write`, `_owned_supervisor_cancellation`, `_get_or_submit_supervisor_cancellation`, and `stop` retain actual `concurrent.futures.Future` handles through timeout, cancellation, and bounded shutdown. On the durable-identity production path, the supervisor operation is bound to exact process proof; legacy compatibility doubles without that API remain future-owned but are explicitly non-attested. `tests/test_observation_service.py::test_supervisor_timeout_remains_owned_through_shutdown`, `tests/test_observation_service.py::test_supervisor_timeout_does_not_overlap_next_recovery`, `tests/test_observation_service.py::test_late_supervisor_result_does_not_terminalize_stale_recovery`, and `tests/test_observation_service.py::test_late_supervisor_failure_is_consumed_and_visible` prove process-cancellation ownership. `tests/test_observation_service.py::test_late_durable_write_remains_owned_through_bounded_shutdown`, `tests/test_observation_service.py::test_caller_cancellation_does_not_cancel_started_durable_write`, `tests/test_observation_service.py::test_conflicting_pending_durable_write_payload_is_rejected_without_second_invocation`, `tests/test_observation_service.py::test_late_durable_recovery_write_remains_visible_to_fresh_service`, `tests/test_observation_service.py::test_cancellation_resistant_non_write_worker_remains_owned_and_fences_restart`, and `tests/test_observation_service.py::test_fresh_service_uses_durable_recovery_when_completion_callback_loop_is_unavailable` prove the database/lifecycle boundary. `backend/tests/test_execution_launch_inventory.py::test_worker_signals_during_active_handler_finish_work_then_exit` covers awaited signal shutdown. | Local evidence added; OPEN pending CI, independent acceptance, and non-empty deployed recovery proof |
...
For the Contract 08 §6.1.2 row above, the current candidate's shared
`execution_id` plus `organization_id` pending-operation gate is verified by
`tests/test_observation_service.py::test_uncertain_supervisor_timeout_does_not_claim_second_recovery_lease`.
The vector proves that a blocked uncertain supervisor future leaves the
durable `DEFERRED` projection at attempt 1 with no owner or lease and no new
attempt or supervisor invocation; after the original future is consumed, a
later eligible cadence creates attempt 2 without overlap. This remains local
evidence and the row remains OPEN pending CI, deployment, and independent
acceptance.
