...
- **Status:** Repository controls verified. Approved managed E12 binaries/packages were unavailable in this environment; real-runtime verification is `UNAVAILABLE`, and no unmanaged runtime is treated as evidence. Authenticated external CLI injection is intentionally fail-closed; governed native HTTP coverage handles tenant credentials until secret-safe subprocess injection exists.
...
- **Status:** `REWORK IN PROGRESS` after independent review identified shared process-supervision, fallback-provenance, execution-state attribution, evidence-sanitization, and authoritative-persistence defects. Corrections are implemented and verified by focused tests and full regression. The fresh Linux production image verified managed trust records and runtime paths for Nuclei, FFuF, Gitleaks, Subfinder, httpx, Katana, Syft, Grype, OSV-Scanner, TruffleHog, Dockle, kube-bench, Trivy, and Retire.js under unprivileged uid 999. Approved managed Python runtime evidence for Semgrep and Bandit remains incomplete, and no unmanaged runtime is treated as evidence.
