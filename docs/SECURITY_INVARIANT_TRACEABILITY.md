...
- **Status:** VERIFIED for the non-escalation and explicit-admission boundaries and for exact destinations in platform-owned CT clients; Prowler worker credentials now require a typed organization/asset/provider-bound, expiring envelope. OS-level external-process egress governance and a production credential-provider backend remain pending capabilities.
