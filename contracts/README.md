...
| [`03_ENGINE_PLUGIN_INTERFACE_CONTRACT.md`](./03_ENGINE_PLUGIN_INTERFACE_CONTRACT.md) | **Engine Plugin Interface** | Standardized `BaseAssessmentEngine` abstract interface, error isolation, and detailed specifications for all 5 security engines (`network`, `web_dast`, `code_sast`, `infra_iac`, `cicd_audit`). |
| [`04_API_AND_STREAMING_EVENTS_CONTRACT.md`](./04_API_AND_STREAMING_EVENTS_CONTRACT.md) | **REST API & Streaming Events** | Complete REST endpoints (`/api/scans/*`, `/api/reports/*`, `/api/system/*`) and real-time SSE streaming event specifications. |
| [`05_DELIVERABLES_AND_ACCEPTANCE_CRITERIA_CONTRACT.md`](./05_DELIVERABLES_AND_ACCEPTANCE_CRITERIA_CONTRACT.md) | **Deliverables & Acceptance Criteria** | Deliverables checklist, 10 concrete acceptance test scenarios (including multi-page crawling and authenticated DAST sessions), and strict Definition of Done (DoD). |
...
| [`07_FRONTEND_UI_UX_SPECIFICATION_CONTRACT.md`](./07_FRONTEND_UI_UX_SPECIFICATION_CONTRACT.md) | **Frontend UI/UX & Design System** | Cyber SOC Dark Theme tokens, HUD component hierarchy, real-time log terminal streamer, scorecard widget, and SSE protocol manager. |
| [`08_TECHNICAL_IMPLEMENTATION_AND_TEST_VECTORS_CONTRACT.md`](./08_TECHNICAL_IMPLEMENTATION_AND_TEST_VECTORS_CONTRACT.md) | **Technical Implementation & Test Vectors** | Exact execution algorithms, socket/X.509 logic, Shannon entropy formula, 40+ secret regex patterns, AST rules, and standardized code remediation templates. |
