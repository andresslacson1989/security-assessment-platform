...
- [ ] **Automated Test Suite (`/tests`):** Comprehensive `pytest` test suite with 100% engine check coverage across all 5 engines and 20 acceptance scenarios.
...
- [ ] **Production Multi-Stage Dockerfile (`Dockerfile`):** Hardened Debian/Python 3.11 base pre-packaging all 10 tools, CPAN Perl modules (`libxml-writer-perl`), non-root `appuser`, healthcheck endpoint probe, and layer caching.
...
- [ ] **Automated Multi-Architecture CI/CD Workflow (`.github/workflows/docker-publish.yml`):** GitHub Actions pipeline building multi-arch (`linux/amd64`, `linux/arm64`) images and publishing to GitHub Container Registry (`ghcr.io`) and Docker Hub.

---

## 2. Rigorous Acceptance Test Scenarios

The platform must pass all 20 acceptance test scenarios deterministically:
...
20. **Scenario 20: Production Containerization, Health Probes & 10-Tool Pre-installation Parity**
    - Verifies that the container image builds deterministically, boots the FastAPI server, passes `/api/system/health` probe, and reports all 10 tools (`nmap`, `nikto`, `nuclei`, `ffuf`, `gitleaks`, `trivy`, `bandit`, `sslyze`, `semgrep`, `checkov`) as `INSTALLED` and functional out-of-the-box.
...
7. `pytest tests/ -v` passes with 100% success rate across all 20 scenarios.
